...
class ShinxStrategy(BaseClass):    
    def __init__(self):
        ...
        self.pricesb=[]
        self.pricesa=[]
        super().__init__("SHINX", 60) # Initialize using the init dunder from its Parent class
        self.lookback = 20
    def get_orders(self, state, orderbook, position):
        orders = []
        ...
        mid_price = (best_ask + best_bid) // 2
        self.prices.append(mid_price)
        self.pricesb.append(best_bid)
        self.pricesa.append(best_ask)
        l=len(self.pricesb)
        if len(self.pricesb) > self.lookback:
            momentum = self.pricesb[l-1] - self.pricesb[l-1-15]
            if momentum > 0:
                orders.append(Order(self.product_name, self.pricesb[l-1], self.max_position - position))   # price is higher than 'lookback' time units ago
            else:
                orders.append(Order(self.product_name, self.pricesa[l-1], -self.max_position + position))  # price is lower, downtrend may continue
        return orders
class AshStrategy(BaseClass):    
    def __init__(self):
        ...
        self.pricesb=[]
        self.pricesa=[]
        super().__init__("ASH", 60) # Initialize using the init dunder from its Parent class
        self.lookback = 20
    def get_orders(self, state, orderbook, position):
        orders = []
...
class JolteonStrategy(BaseClass):    
    def __init__(self):
        ...
        self.pricesb=[]
        self.pricesa=[]
        super().__init__("JOLTEON", 350) # Initialize using the init dunder from its Parent class
        self.lookback = 20
    def get_orders(self, state, orderbook, position):
        orders = []
...
class LuxrayStrategy(BaseClass):    
    def __init__(self):
        ...
        self.pricesb=[]
        self.pricesa=[]
        super().__init__("LUXRAY", 250) # Initialize using the init dunder from its Parent class
        self.lookback = 20
    def get_orders(self, state, orderbook, position):
        orders = []
...
class MistyStrategy(BaseClass):    
    def __init__(self):
        ...
        self.pricesb=[]
        self.pricesa=[]
        super().__init__("MISTY", 100) # Initialize using the init dunder from its Parent class
        self.lookback = 20
    def get_orders(self, state, orderbook, position):
        orders = []
        ...
        return orders
class Trader:
    MAX_LIMIT = 800 # for single product mode only, don't remove
    def __init__(self):
        self.strategies = {
